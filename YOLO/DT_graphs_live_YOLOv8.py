...
def run_yolo():
    ...
    while time.time() - start_time < 10:  # Stream przez 30 sekund
        ret, frame = cap.read()
        if not ret:
            break
        ...
        cv2.imshow("Detekcja YOLOv8", frame)
...
def generate_plots():
    ...
    for label, values in confidence_dict.items():
        ...
        # Zaznaczanie najmniejszej i największej wartości
        min_conf = min(confidence_scores)
        max_conf = max(confidence_scores)
        
        # Dodanie poziomej linii dla minimalnej wartości
        plt.axhline(min_conf, color='red', linestyle='--', label=f'Min: {min_conf:.2f}')
        
        # Dodanie poziomej linii dla maksymalnej wartości
