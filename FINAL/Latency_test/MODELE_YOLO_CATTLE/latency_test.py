...
# 🔧 === USTAWIENIA ŚCIEŻEK (zmień je na swoje lokalizacje) === 🔧
...
EFFDET_FILE_PATH = "/mnt/d/home/miko/Latency_test/MODELE_YOLO_CATTLE/dane_mAP_effdet.txt"
...
def parse_effdet_file(filepath):
    effdet_models = []
    effdet_latencies = []
    effdet_map50 = []
    effdet_map5090 = []
    ...
    for line in lines:
        ...
        if len(parts) >= 5:
            try:
                effdet_models.append(parts[0])
                effdet_map50.append(float(parts[1]))
                effdet_map5090.append(float(parts[2]))
                effdet_latencies.append(float(parts[3]))
            except ValueError:
                continue

    return effdet_models, effdet_latencies, effdet_map50, effdet_map5090


def plot_graph(latencies, map_values, model_names, ylabel, title, filename, model_groups, colors,
               effdet_names, effdet_lat, effdet_map50, effdet_map5090):
    ...
    for group_idx, (group, color) in enumerate(zip(model_groups, colors)):
        ...
        if group_idx == len(model_groups) - 1:  # EffDet
            group_latencies = effdet_lat
            group_map_values = effdet_map50 if '50' in ylabel and '95' not in ylabel else effdet_map5090
            plt.plot(group_latencies, group_map_values, '-o', color=color, markerfacecolor='white', markeredgewidth=2)
            for x, y, name in zip(group_latencies, group_map_values, effdet_names):
    ...
    plt.xlabel('Latency (FPS)', fontsize=12)
    ...
    for idx, group in enumerate(model_groups):
        ...
        if idx == len(model_groups) - 1:
            legend_labels.append("EffDet")
        else:
            label = group[0].split('-')[0].upper() + " models"
...
def main():
    ...
    effdet_names, effdet_lat, effdet_map50, effdet_map5090 = parse_effdet_file(EFFDET_FILE_PATH)
    ...
    for model_file in model_files:
        ...
        print(f"   🚀 Średnia latencja: {latency:.2f} FPS")
    ...
    model_groups = [yolov8_models, yolov5_models, v11_models, v12_models, effdet_models]
    ...
    plot_graph(
        ...
        colors=colors,
        effdet_names=effdet_names,
        effdet_lat=effdet_lat,
        effdet_map50=effdet_map50,
        effdet_map5090=effdet_map5090
    )
    ...
    with open(OUTPUT_RESULTS_PATH, 'w') as f:
        ...
        for name, lat, m50, m5090 in zip(effdet_names, effdet_lat, effdet_map50, effdet_map5090):
            f.write(f"{name}, {lat:.2f}, {m50:.3f}, {m5090:.3f}\n")
